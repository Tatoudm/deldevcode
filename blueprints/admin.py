...
def admin_new_discount():
    ...
    active_discounts = []
    try:
        active_discounts = list(d_col.find({"active": True}).sort("created_at", -1))
    except Exception:
        active_discounts = []
...
def admin_support_detail(ticket_id):
    ...
    ticket_view = {
        ...
        "subject_label": ticket.get("subject_label")
...
def admin_user_action():
    ...
    if action == "make_admin":
        ...
        minutes_str = (request.form.get("minutes") or "").strip() or (
            request.form.get("duration") or ""
        ).strip()
        try:
        ...
        mute_reason = (request.form.get("reason") or "").strip() or (
            request.form.get("mute_reason") or ""
        ).strip()
        ...
        u_col.update_one({"_id": user["_id"]}, {"$set": {"warns_count": current_warns}})
