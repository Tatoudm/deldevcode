        ...
        if action =="apply_code":
            if not promo_code_value :
                ...
                try :
                    ...
                    error ="Erreur lors de la vérification du code. Réessaie plus tard."
    else :
...
def create_checkout_session ():
    ...
    if promo_id :
        ...
        if auto_discount and auto_discount .get ("stripe_coupon_id"):
            discounts_for_stripe .append (
            {"coupon":auto_discount ["stripe_coupon_id"]}
            )
...
def stripe_webhook ():
    ...
    if event_type =="checkout.session.completed":
        ...
        if mode =="subscription":
            user_id_str =data_object .get ("client_reference_id")or data_object .get ("metadata",{}).get ("user_id")
