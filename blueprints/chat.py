...
def chat():
    ...
    if util.get("muted", 0) == 1:
        ...
        if muted_until is None or muted_until > now:
            ...
            mute_message = (
                f"Tu as été mute {durée_txt } par le modérateur : {muted_by }."
            )
        else:
    ...
    for g in groups_cursor:
        ...
        if (
            last_msg
            and last_msg.get("created_at")
            and last_msg["created_at"] > last_seen_dm
        ):
...
def create_announcement():
    ...
    if len(title) > 100:
        session["chat_error"] = (
            "Le titre de l'annonce est trop long (100 caractères max)."
        )
        return redirect(url_for("chat.chat"))

    if len(description) > 1000:
        session["chat_error"] = (
            "La description de l'annonce est trop longue (1000 caractères max)."
        )
        return redirect(url_for("chat.chat"))
...
def delete_announcement(ann_id):
    ...
    if author == "tatoudm" and not is_superadmin():
        session["chat_error"] = (
            "Seul le superadmin peut supprimer ses propres annonces."
        )
        return redirect(url_for("chat.chat"))
...
def rename_group(group_id):
    ...
    g_col.update_one({"_id": group["_id"]}, {"$set": {"name": new_name}})
...
def kick_group_member(group_id):
    ...
    if not is_effective_group_owner(username, group):
        session["chat_error"] = (
            "Seul le propriétaire du groupe peut exclure des membres."
        )
        return redirect(url_for("chat.chat"))
...
def toggle_group_invites(group_id):
    ...
    if not is_effective_group_owner(username, group):
        session["chat_error"] = (
            "Seul le propriétaire du groupe peut changer ce paramètre."
        )
        return redirect(url_for("chat.chat"))
